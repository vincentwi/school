...
We can see from the graph that the distributions look like two distinct distributions. With this, qualitatively we can say that taking aspirin has a significant and independent effect, but to quantify this we used several methods: zscore and confidence interval.
...
cauchy <- function(x_0=0, gamma=1.0, size=1000)
{
  ...
  true_exponential <- rexp(size, r=gamma) + x_0
...
$$
We choose this as our estimator because, although s4 is unbiased compared to s5, s5 has a much faster rate of convergence. We can see, through an application of the continuous map theorem, where the MSE of s4 is $$\frac{\sigma^2}{n}$$ and s5 = $$\frac{\sigma^4}{n^2}$$ $$4\sigma^2 \mu^2$$, assuming there is no degenerate solution $$\mu=0$$ (in which case we just use the standard gaussion distribution under CLT with $$N(0,\sigma^2)$$),
c) 
